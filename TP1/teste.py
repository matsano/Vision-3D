...
for p in range(points_selected):
	x  = X_init[p][0]
	y  = X_init[p][1]
	xl = X_final[p][0]
	yl = X_final[p][1]
	A[p] = [-x, -y, -1, 0, 0, 0, x*xl, xl*y, xl]
...
for p in range(points_selected):
	...
	xl = X_final[p][0]
	yl = X_final[p][1]
	A[p+points_selected] = [0, 0, 0, -x, -y, -1, yl*x, yl*y, yl]
...
# get last collum from V
# reshape into 3x3 to create H
...
cv2.imshow("Image rectifie",img_warp)
cv2.waitKey(0)